...
async function insert_teams(arr, arr2, coordinator_id) {
    ...
    for (var j = 0; j < arr2.length; j++) {
        try {
            ...
            var result = await sequelize.query(`Select user_id from users where first_name= ? and last_name=?`, { replacements: [arr2[j].first_name, arr2[j].last_name], type: sequelize.QueryTypes.CALL })
                //Name not found
